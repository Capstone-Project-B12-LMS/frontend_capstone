...
const Layout = () => {
  ...
  const [creating, { loading: creatingLoad }] = useMutation(CREATE_CLASS, {
    onCompleted: (data) =>
      navigateToClass(`teacher/${data?.class?.save?.id}`, setCreateClassShow),
    onError: () => {},
  ...
  const [joined, { loading: joinLoading }] = useMutation(JOIN_CLASS, {
    onCompleted: (data) =>
      navigateToClass(`class/${data?.class?.join?.id}`, setjoinClassShow),
    onError: () => {},
    notifyOnNetworkStatusChange: true,
  });
  ...
  const showPopupCreate = (e, show = !createClassShow) =>
    setCreateClassShow(show);
  ...
  const createClass = (e) => {
    ...
    const matchClassname =
      classname.value.trim() !== "" && classname.pattern.test(classname.value);
      ...
      >
        <form onSubmit={createClass} className="mt-6 w-full">
          ...
          <Button
            formBtn={true}
            styling={
              "rounded-[15px] w-full mt-8 h-[62px] text-[20px] font-bold flex justify-center items-center"
            }
            text={"Create"}
      ...
      <div className="px-10 mx-auto max-w-[1600px]">
        <Routes>
          <Route
            path="home"
            element={
              <Home joinClass={showPopupJoin} createClass={showPopupCreate} />
            }
          />
