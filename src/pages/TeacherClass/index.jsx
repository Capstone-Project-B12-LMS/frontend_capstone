...
const TeacherClass = () => {
  ...
  // Graphql
  const { data: dataCounseling, loading: loadingCounseling } = useGetCounseling(
    params.id,
  );
  ...
  const { data: dataMaterial, loading: loadingMaterial } = useQuery(
    FIND_CLASS_MATERIAL,
    { variables: { class_id: params.id } },
  );
  ...
  const targetMaterial = dataMaterial?.material.findAllByClassId.filter(
  ...
  useEffect(() => {
    if (!!materialId) {
      ...
      setMaterial({
        ...
        linkVideo: filterMaterial[0].videoUrl
          ? `https://youtu.be/${filterMaterial[0].videoUrl}`
          : null,
        linkPowerPoint: filterMaterial[0].videoUrl
          ? filterMaterial[0].fileUrl
          : null,
      });
      setUpdateMode(true);
    }
                ...
                <div className="border-[1px] rounded-[10px] p-6">
                  ...
                  {!loadingCounseling &&
                    dataCounseling.guidance.findByClassId.map((counsel) => (
                      <Counseling data={counsel} key={counsel.id} />
                    ))}
                  {!loadingCounseling && isViewClicked && (
                    <ViewPopUp
                      setIsViewClicked={setIsViewClicked}
                      dataCounseling={dataCounseling.guidance.findByClassId}
                    />
                  )}
                </div>
              ) : (
            ...
            <div className="w-[75%]">
              <div>
                <Routes>
                  ...
                  <Route
                    path="feedback"
                    element={<Feedback id_class={data?.class.findById.id} />}
                  />
                  <Route
                    path="setting/*"
                    element={<Setting dataClass={data} func={setMaterialId} />}
                  />
                </Routes>
              </div>
            </div>
