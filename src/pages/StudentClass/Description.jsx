...
const Description = ()=>{
    ...
    const teacher = data?.class?.findById?.users.filter(user => user.email === data?.class?.findById?.createdBy); 
        ...
        <>
            ...
            <div className="bg-white border border-solid border-[#A8A8A8] rounded-[20px] py-6 mt-6">
                <div className="flex justify-between px-6">
                    ...
                    <span className="text-black font-normal text-xl">{ data?.class?.findById?.users.length > 20 ? `${data?.class?.findById?.users.length}+ Members` : `${data?.class?.findById?.users.length} Members` }</span>
                </div>
                ...
                <div className='mt-10 px-6'>
                    ...
                    <div className='mt-6 min-h-[150px] max-h-[250px] overflow-y-auto'>
                        {
                            data?.class?.findById?.users.map(member => (
                                <UserList 
                                    key={member.id}
                                    avatar="https://i.ibb.co/nwbqL4K/7ba8ec4a42b529dcbbc695ce0dd07a4a.jpg"
